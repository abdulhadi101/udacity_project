...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user is None or not user.check_password(form.password.data):
            ...
            # logging for invalid logging
            app.logger.warning('Invalid Logging Attempt')
...
def authorized():
    ...
    if request.args.get('code'):
        ...
        result = _build_msal_app(cache=cache).acquire_token_by_authorization_code(request.args['code'],
                                                                                  scopes=Config.SCOPE, redirect_uri=url_for('authorized',
...
def _build_auth_url(authority=None, scopes=None, state=None):
    ...
    return _build_msal_app(authority=authority).get_authorization_request_url(scopes or [],
                                                                              state=state or str(uuid.uuid4()), redirect_uri=url_for('authorized', _external=True, _scheme='https'))
